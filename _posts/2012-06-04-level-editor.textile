...
Each %(struct)tile% can contain own %(struct)properties%. Tiles are referenced by id=GID. Tileset's tile properties contains global for all objects, using this tileset's tile(s), properties. These can be adjusted in %(struct)ObjectGroups'% %(struct)object% properties. %(struct)Tile% is a matter for %(struct)ObjectPrototype% of the game.
...
* %(struct)object% - describes single object on the map. Properties describe per-object special properties.
